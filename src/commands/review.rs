...
pub fn handle(args: &ReviewArgs) -> Result<()> {
    if args.list_sounds {
    ...
    // Resolve target strictly from flags and anchor at the crate's blueprints directory
    ...
    let prompt = Profile::Review
        .compose()
        .with_blueprints_dir(blueprints.dir_token_value())
        .inline_blueprints()
        .build()?;

    // Run codex with reviewer profile; allow writes so the agent can apply fixes
